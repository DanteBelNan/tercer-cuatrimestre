use dos_dos
-- Rehacer los procedimientos almacenados B-D para que se ejecuten dentro de una transacción.
...
CREATE PROCEDURE sp_Agregar_UsuarioT(
    @DNI INT,
    @Apellido VARCHAR(50),
    @Nombre VARCHAR(50),
    @nacimiento DATETIME,
...
CREATE PROCEDURE sp_Agregar_TarjetaT(
    @dni BIGINT
)
AS
BEGIN
...
CREATE PROCEDURE sp_Agregar_ViajeT(
    @nroTarjeta int,
...
CREATE PROCEDURE sp_Agregar_SaldoT
    @nroTarjeta INT,
...
CREATE PROCEDURE sp_Baja_Fisica_UsuarioT(
    @dni INT
)
AS
BEGIN
...
CREATE PROCEDURE sp_Baja_Logica_UsuarioT(
    @dni INT
)
AS
BEGIN
